...
public class GenomeNexusUtils {
    ...
    public static TranscriptMatchResult matchTranscript(TranscriptPair transcript, ReferenceGenome referenceGenome,
            String hugoSymbol) throws ApiException {
        ...
        if (_ensemblTranscript.isPresent()) {
            ...
            if (StringUtils.isNotEmpty(_ensemblTranscript.get().getProteinId())) {
                _sequence = getProteinSequence(transcript.getReferenceGenome(),
                        _ensemblTranscript.get().getProteinId());
            }
            if (_sequence.isPresent()) {
                List<EnsemblTranscript> targetEnsemblTranscripts = getEnsemblTranscriptList(hugoSymbol,
                        referenceGenome);
                if (targetEnsemblTranscripts.size() == 0) {
                    ...
                    try {
                        pickEnsemblTranscript(transcriptMatchResult, referenceGenome, targetEnsemblTranscripts,
                                _sequence.get());
                ...
                transcriptMatchResult.setNote(
                        "No protein sequence available for transcript " + _ensemblTranscript.get().getTranscriptId());
            }
        } else {
    ...
    public static TranscriptConsequenceSummary getTranscriptConsequence(GNVariantAnnotationType type, String query,
        ...
        if (type == GNVariantAnnotationType.GENOMIC_LOCATION) {
            annotations.addAll(getGenomicLocationVariantsAnnotation(
    ...
    public static List<TranscriptConsequenceSummary> getTranscriptsConsequence(List<VariantAnnotation> annotations,
            ReferenceGenome referenceGenome) throws ApiException {
    ...
    public static List<GenomeNexusAnnotatedVariantInfo> getAnnotatedVariantsFromGenomeNexus(
            GNVariantAnnotationType type, List<VariantAnnotation> variantsAnnotation, ReferenceGenome referenceGenome)
            throws ApiException {
        ...
        for (VariantAnnotation annotation : variantsAnnotation) {
            ...
            if (annotation != null) {
                ...
                // Use Genome Nexus annotation to translate original query to HGVSg or Genomic
                // Location
                ...
                if (type == GNVariantAnnotationType.HGVS_G) {
                    if (annotation.getAnnotationSummary() != null
                            && annotation.getAnnotationSummary().getGenomicLocation() != null) {
                        org.genome_nexus.client.GenomicLocation gl = annotation.getAnnotationSummary()
                                .getGenomicLocation();
                        if (gl.getChromosome() != null && gl.getStart() != null && gl.getEnd() != null
                                && gl.getReferenceAllele() != null && gl.getVariantAllele() != null) {
                            String glString = gl.getChromosome() + "," + gl.getStart() + "," + gl.getEnd() + ","
                                    + gl.getReferenceAllele() + "," + gl.getVariantAllele();
            ...
            if (transcriptConsequenceSummary != null) {
                ...
                if (transcriptConsequenceSummary.getProteinPosition() != null) {
                    preAnnotatedVariantInfo
                            .setProteinStart(transcriptConsequenceSummary.getProteinPosition().getStart());
                }
                if (transcriptConsequenceSummary.getProteinPosition() != null) {
    ...
    public static List<VariantAnnotation> getHgvsgVariantsAnnotation(List<String> queries,
            ReferenceGenome referenceGenome) throws ApiException {
        List<VariantAnnotation> variantsAnnotation = new ArrayList<>();
        if (queries != null) {
            List<String> gnFields = new ArrayList<>();
            gnFields.add("annotation_summary");
            variantsAnnotation = getAnnotationControllerApi(referenceGenome).fetchVariantAnnotationPOST(queries,
                    MSK_ISOFORM_OVERRIDE, null, gnFields);
        }
        return variantsAnnotation;
    }

    public static List<VariantAnnotation> getGenomicLocationVariantsAnnotation(List<GenomicLocation> queries,
            ReferenceGenome referenceGenome) throws ApiException {
        List<VariantAnnotation> variantsAnnotation = new ArrayList<>();
        if (queries != null) {
            List<String> gnFields = new ArrayList<>();
            gnFields.add("annotation_summary");
            variantsAnnotation = getAnnotationControllerApi(referenceGenome)
                    .fetchVariantAnnotationByGenomicLocationPOST(queries, MSK_ISOFORM_OVERRIDE, null, gnFields);
        }
        return variantsAnnotation;
    }

    private static TranscriptConsequenceSummary getConsequence(VariantAnnotation variantAnnotation,
            ReferenceGenome referenceGenome) {
        List<TranscriptConsequenceSummary> summaries = new ArrayList<>();

        if (variantAnnotation == null || variantAnnotation.getAnnotationSummary() == null
                || variantAnnotation.getAnnotationSummary().getTranscriptConsequenceSummaries() == null) {
            return null;
        }

        if (variantAnnotation.getAnnotationSummary() != null
                && variantAnnotation.getAnnotationSummary().getTranscriptConsequenceSummaries() != null) {
            for (TranscriptConsequenceSummary consequenceSummary : variantAnnotation.getAnnotationSummary()
                ...
                if (StringUtils.isNotEmpty(consequenceSummary.getTranscriptId())) {
                    ...
                    if (gene != null
                            && (StringUtils.isEmpty(isoform) || isoform.equals(consequenceSummary.getTranscriptId()))) {
        ...
        if (summaries.size() == 1) {
            ...
            summary = pickTranscriptConsequenceSummary(summaries,
                    variantAnnotation.getAnnotationSummary().getCanonicalTranscriptId(),
                    variantAnnotation.getMostSevereConsequence());
        }
        // no transcript marked as canonical (list.size() == 0), use most sever
        // consequence to decide which one to pick among all available
        else {
            summary = pickTranscriptConsequenceSummary(
                    variantAnnotation.getAnnotationSummary().getTranscriptConsequenceSummaries(),
                    variantAnnotation.getAnnotationSummary().getCanonicalTranscriptId(),
                    variantAnnotation.getMostSevereConsequence());
        }
        ...
        if (summary != null) {
            VariantConsequence consequence = consequenceResolver(summary.getConsequenceTerms(),
                    summary.getVariantClassification());
    ...
    private static TranscriptConsequenceSummary pickTranscriptConsequenceSummary(
            List<TranscriptConsequenceSummary> summaries, String canonicalTranscript, String mostSevereConsequence) {
        if (summaries == null) {
            return null;
        }
        // Find canonical isoforms first
        List<TranscriptConsequenceSummary> canonicalTranscripts = summaries.stream()
                .filter(summary -> StringUtils.isNotEmpty(summary.getTranscriptId())
                        && summary.getTranscriptId().equals(canonicalTranscript))
                .collect(Collectors.toList());

        if (StringUtils.isNotEmpty(mostSevereConsequence)) {
            canonicalTranscripts = canonicalTranscripts.stream()
                    .filter(summary -> StringUtils.isNotEmpty(summary.getConsequenceTerms())
                            && Arrays.asList(summary.getConsequenceTerms().split(",")).contains(mostSevereConsequence))
                    .collect(Collectors.toList());
        }
    ...
    public static List<EnsemblTranscript> getEnsemblTranscriptList(List<String> ensembelTranscriptIds,
            ReferenceGenome referenceGenome) throws ApiException {
    ...
    private static List<EnsemblTranscript> getEnsemblTranscriptList(String hugoSymbol, ReferenceGenome referenceGenome)
            throws ApiException {
        EnsemblControllerApi controllerApi = GenomeNexusUtils.getEnsemblControllerApi(referenceGenome);
    ...
    public static EnsemblTranscript getCanonicalEnsemblTranscript(String hugoSymbol, ReferenceGenome referenceGenome)
            throws ApiException {
        EnsemblControllerApi controllerApi = GenomeNexusUtils.getEnsemblControllerApi(referenceGenome);
        try {
    ...
    private static Optional<EnsemblTranscript> getEnsemblTranscript(String hugoSymbol, TranscriptPair transcriptPair)
            throws ApiException {
        return getEnsemblTranscriptList(hugoSymbol, transcriptPair.getReferenceGenome()).stream()
                .filter(ensemblTranscript -> !StringUtils.isEmpty(ensemblTranscript.getTranscriptId())
                        && ensemblTranscript.getTranscriptId().equalsIgnoreCase(transcriptPair.getTranscript()))
                .findFirst();
    }
    ...
    private static Optional<Sequence> getProteinSequence(ReferenceGenome referenceGenome, String transcript) {
        ...
        ResponseEntity<Sequence> response = restTemplate.exchange(
                GenomeNexusUtils.getEnsemblSequenceGETUrl(referenceGenome, transcript), HttpMethod.GET, entity,
                Sequence.class);
    ...
    private static TranscriptMatchResult pickEnsemblTranscript(TranscriptMatchResult transcriptMatchResult,
            ReferenceGenome referenceGenome, List<EnsemblTranscript> availableTranscripts, Sequence sequence) {
        List<EnsemblTranscript> sameLengthList = availableTranscripts.stream()
                .filter(ensemblTranscript -> ensemblTranscript.getProteinLength() != null
                        && ensemblTranscript.getProteinLength().equals(sequence.getSeq().length()))
                .collect(Collectors.toList());

        List<Sequence> sequences = getProteinSequences(referenceGenome,
                sameLengthList.stream().map(EnsemblTranscript::getProteinId).collect(Collectors.toList())).stream()
                .filter(filteredSequence -> filteredSequence.getSeq().length() == sequence.getSeq().length())
                .collect(Collectors.toList());
        Optional<Sequence> sequenceSame = sequences.stream()
                .filter(matchedSequence -> matchedSequence.getSeq().equals(sequence.getSeq())).findAny();

        if (sequenceSame.isPresent()) {
            Optional<EnsemblTranscript> ensemblTranscript = getEnsemblTranscriptBySequence(sameLengthList,
                    sequenceSame.get());
            ...
            sequences.sort(
                    Comparator.comparingInt(s -> getNumOfMismatchSameLengthSequences(sequence.getSeq(), s.getSeq())));
            Sequence pickedSequence = sequences.iterator().next();

            Optional<EnsemblTranscript> ensemblTranscript = getEnsemblTranscriptBySequence(availableTranscripts,
                    pickedSequence);
            transcriptMatchResult.setTargetEnsemblTranscript(ensemblTranscript.get());
            transcriptMatchResult.setNote("Same length, but mismatch: "
                    + getNumOfMismatchSameLengthSequences(sequence.getSeq(), pickedSequence.getSeq()));
        } else {
            // we want to see whether there is any transcript includes the original sequence
            List<EnsemblTranscript> longerOnes = availableTranscripts.stream()
                    .filter(ensemblTranscript -> ensemblTranscript.getProteinLength() != null
                            && ensemblTranscript.getProteinLength() > sequence.getSeq().length())
                    .collect(Collectors.toList());

            List<Sequence> longerSequences = getProteinSequences(referenceGenome,
                    longerOnes.stream().map(EnsemblTranscript::getProteinId).collect(Collectors.toList()));
            List<Sequence> sequencesContains = longerSequences.stream()
            ...
            if (sequencesContains.size() > 0) {
                ...
                Optional<EnsemblTranscript> ensemblTranscript = getEnsemblTranscriptBySequence(longerOnes,
                        pickedSequence);
                transcriptMatchResult.setTargetEnsemblTranscript(ensemblTranscript.get());
                transcriptMatchResult
                        .setNote("Longer one found, length: " + ensemblTranscript.get().getProteinLength());
    ...
    private static Optional<EnsemblTranscript> getEnsemblTranscriptBySequence(
            List<EnsemblTranscript> availableEnsemblTranscripts, Sequence sequence) {
