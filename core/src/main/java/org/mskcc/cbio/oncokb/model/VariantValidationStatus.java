...
/**
 * Severity of the outcome of validating a queried variant (a protein change against the OncoKB
 * canonical protein sequence, or the separator of a fusion name). This is distinct from {@link VariantAnnotationMessageType}, which names the
 * specific reason; the status only tells the frontend how to treat the result.
 */
public enum VariantValidationStatus {
    ...
    // position, reversed range, malformed form), or names a fusion whose partners cannot be told
    // apart. The detail is carried in the message.
    INVALID,
