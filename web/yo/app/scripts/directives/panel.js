...
angular.module('oncokbApp')
    .directive('panel', function($rootScope, DatabaseConnector, dialogs, _, OncoKB, mainUtils, ReviewResource) {
        return {
            ...
            controller: function($scope) {
                ...
                $scope.signatureCheck = function() {
                    ...
                    if ($scope.uuid && ReviewResource.mostRecent[$scope.uuid]) {
                        ...
                        $scope.updatedBy = $scope.data[$scope.key+'_review'].updatedBy;
                        $scope.updateTime = $scope.data[$scope.key+'_review'].updateTime;
                    }
                ...
                function rejectItem(arr) {
                    if (arr.length === 0) {
                        if(mainUtils.needReviewFire($scope.uuid)) {
                            ...
                            $scope.data[$scope.key+'_review'].updateTime = tempTime;
                            delete $rootScope.metaFire[$scope.uuid];
                            ReviewResource.rejected.push($scope.uuid);
                        }
                    } else {
                        _.each(arr, function(childKey) {
                            if(mainUtils.needReviewFire($scope.data[$scope.key][childKey+'_uuid'])) {
                                $scope.data[$scope.key][childKey] = $scope.data[$scope.key][childKey+'_review'].lastReviewed;
                                var tempTime = $scope.lastUpdateTime;
                                if(!tempTime) {
                                    tempTime = new Date().getTime();
                                }
                                delete $scope.data[$scope.key][childKey+'_review'].lastReviewed;
                                $scope.data[$scope.key][childKey+'_review'].updateTime = tempTime;
                                delete $rootScope.metaFire[$scope.data[$scope.key][childKey+'_uuid']];
                                ReviewResource.rejected.push($scope.data[$scope.key][childKey+'_uuid']);
                            }
                        });
                    }
