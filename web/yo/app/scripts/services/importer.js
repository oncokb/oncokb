    ...
    .service('importer', function importer($timeout, documents, S, storage, OncoKB, $q, _, stringUtils) {
        ...
        function getGeneData(realtime, excludeObsolete) {
            ...
            geneData.mutations.asArray().forEach(function (e) {
                if (!(excludeObsolete !== undefined && excludeObsolete && e.name_eStatus && e.name_eStatus.has('obsolete') && e.name_eStatus.get('obsolete') === 'true' && mutation.oncogenic_eStatus.get('curated')===false)){
                    var _mutation = {};
                    _mutation.tumors = [];
                    _mutation.effect = {};
