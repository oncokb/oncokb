...
public class TrialsApiController {
    ...
    public ResponseEntity<List<ClinicalTrial>> trialsMatchingGet(
        ...
        Map<String, Tumor> tumors = ClinicalTrialsUtils
            .getInstance()
            .getAllMappingResult();
        ...
        if (tumors.containsKey(oncoTreeCode)) {
            ...
            List<ClinicalTrial> trial = ClinicalTrialsUtils
                .getInstance()
    ...
    public ResponseEntity<List<ClinicalTrial>> trialsGetByCancerType(
        ...
        @ApiParam(
            value = "Consisted of single/multuple drugs. Support drugs name or drugs' NCIT code. For multiple drugs treatment, use '+' as separator. Example: Everolimus+C95701"
        ) @RequestParam(value = "treatment", required = false) String treatment,
        @ApiParam(
            value = "The address of your location. Support zip code. Must be spcified with country. Example: New York City, NY"
        ) @RequestParam(value = "address", required = false) String address,
        @ApiParam(
            value = "The country of your location. Must be specified with address. Example: United States"
        ) @RequestParam(value = "country", required = false) String country,
        @ApiParam(
            value = "The radius from your location. Must be specified with address and country. Example: 100, which means all trails have any site locates within 100 km from your location. If not specify, the default distance is 100km."
        ...
        Map<String, Tumor> tumors = ClinicalTrialsUtils.getInstance().getAllMappingResult();
        if (cancerType.equals(SpecialTumorType.ALL_TUMORS.toString())) {
            ...
            for (String oncoTreeCode : tumors.keySet()) {
                Tumor tumor = tumors.get(oncoTreeCode);
                for (String curTrialID : tumor.getTrials()) {
                    if (!nctIDSet.contains(curTrialID)) {
                        nctIDSet.add(curTrialID);
                        result.add(ClinicalTrialsUtils.getInstance().getAllTrials().get(curTrialID));
                    }
                }
            }
            result =
        ...
        if (specialTumorType != null) {
            trials =
                ClinicalTrialsUtils
                    .getInstance()
                    .filterTrialsBySpecialCancerType(
                        specialTumorType
                    );
        } else {
            trials =
